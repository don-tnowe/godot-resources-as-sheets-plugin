...
func can_edit_value(value, type, property_hint) -> bool:
	return type == TYPE_PACKED_STRING_ARRAY || type == TYPE_ARRAY
...
func set_value(node : Control, value):
	...
	for i in children.size():
		if i >= value.size():
			...
			children[i].self_modulate = \
				Color.WHITE if !colored else \
				Color(str(value[i]).hash()) + Color(0.25, 0.25, 0.25, 1.0)
