...
func _on_cell_gui_input(event: InputEvent, cell_node: Control):
	...
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			# After checking event is an InputEventMouseButton,
			# which extends InputEventWithModifiers,
			# it should be safe to use is_command_or_control_pressed().
...
func _input(event: InputEvent):
	...
	if (
		...
		or event.keycode == KEY_SHIFT
		or event.keycode == KEY_ALT
	):
		# If the modifier is the only key pressed, do nothing.
		return

	# Ctrl + Z (before, and instead of, committing the action!)
	# After checking event is a InputEventKey, which extends InputEventWithModifiers,
	# it should be safe to use is_command_or_control_pressed().
	if event.is_command_or_control_pressed():
